...
#ifndef phy_id_compare_model
#define phy_id_compare_model(id, model) \
    (((id) & GENMASK(31, 10)) == ((model) & GENMASK(31, 10)))
#endif

#ifndef LINK_INBAND_DISABLE
#define LINK_INBAND_DISABLE     0
#define LINK_INBAND_ENABLE      1
#endif

#ifndef PHY_LED_ACTIVE_HIGH
#define PHY_LED_ACTIVE_HIGH     1
#define PHY_LED_ACTIVE_LOW      0
#endif
...
static int gpy_hwmon_register(struct phy_device *phydev)
{
	...
	hwmon_dev = devm_hwmon_device_register_with_info(dev, "mxl86211c", phydev,
							 info, NULL);
...
static int gpy_loopback_wrapper(struct phy_device *phydev, bool enable)
{
    /* GPY系列不需要speed参数，直接调用 */
    return gpy_loopback(phydev, enable, 0);
}

static int gpy115_loopback_wrapper(struct phy_device *phydev, bool enable)
{
    return gpy115_loopback(phydev, enable, 0);
}
...
static struct phy_driver gpy_drivers[] = {
	{
		...
		.set_wol	= gpy_set_wol,
		.get_wol	= gpy_get_wol,
		.set_loopback	= gpy_loopback_wrapper,
	...
	},
	{
		...
		.set_wol	= gpy_set_wol,
		.get_wol	= gpy_get_wol,
		.set_loopback	= gpy115_loopback_wrapper,
		.led_brightness_set = gpy_led_brightness_set,
		.led_hw_is_supported = gpy_led_hw_is_supported,
		.led_hw_control_get = gpy_led_hw_control_get,
		.led_hw_control_set = gpy_led_hw_control_set,
