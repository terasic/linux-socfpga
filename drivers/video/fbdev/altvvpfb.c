 ...
 */
 ...
 /*
 	2025-06-20 will: 
        - modified for altera vvp framebuffer
    2025-10-23 will: 
        - add altvvpfb_mmap to support kernel 6.12.11-lts
        - change dma_alloc_coherent to dma_alloc_wc
  */
...
// fb_mmap 函数
static int altvvpfb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
    struct altvvpfb_priv *fbpriv = container_of(info, struct altvvpfb_priv, info);
    unsigned long start = vma->vm_start;
    unsigned long size = vma->vm_end - vma->vm_start;
    unsigned long offset = vma->vm_pgoff << PAGE_SHIFT;
    unsigned long phys = info->fix.smem_start + offset;
    int ret;

    /* 检查映射范围是否合法 */
    if (offset + size > info->fix.smem_len) {
        dev_err(info->device, "mmap request out of bounds: offset 0x%lx size 0x%lx total 0x%x\n",
                offset, size, info->fix.smem_len);
        return -EINVAL;
    }

    /* 设置页属性为 write-combined，提高图形性能 */
    vma->vm_page_prot = pgprot_writecombine(vma->vm_page_prot);
    
    /* 映射物理内存到用户空间 */
    ret = remap_pfn_range(vma, start, phys >> PAGE_SHIFT, size, vma->vm_page_prot);
    if (ret) {
        dev_err(info->device, "remap_pfn_range failed: %d\n", ret);
        return ret;
    }

    dev_dbg(info->device, "fb_mmap: phys=0x%lx size=0x%lx\n", phys, size);
    return 0;
}
...
err_dma_free:
	...
	dma_free_wc(dev, fbpriv->info.fix.smem_len, fbmem_virt,fbpriv->info.fix.smem_start);
	return retval;
}
...
void altvvpfb_remove(struct device *dev)
{
	...
	dma_free_wc(dev, fbpriv->info.fix.smem_len,(void *)fbpriv->info.screen_base,fbpriv->info.fix.smem_start);
