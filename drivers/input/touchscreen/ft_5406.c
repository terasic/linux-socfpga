...
static int ft5406_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   INIT_WORK(&ft5406_ts->pen_event_work, ft5406_ts_pen_irq_work);
    ...
    err = of_property_read_u32(np, "pio_phybase", &ts_pio_phybase);
    if (err != 0) {
        err = -EINVAL;
        dev_err(&client->dev,"ft5406_ts_probe: invalid or missing pio_phybash\n");
        goto err_of_property;
    }
    ts_pio_phybase += 0xff200000;
